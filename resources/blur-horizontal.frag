...
const float blurSize = 0.0005;//1.0/1024.0; // I've chosen this size because this will result in that every step will be one pixel wide if the tex0 texture is of size 512x512
...
void main(void)
{
   ...
   sum += texture2D(tex0, vec2(vTexCoord.x + 4.0*blurSize, vTexCoord.y)) * (0.05+0.03);
   ...
   sum += texture2D(tex0, vec2(vTexCoord.x + blurSize, vTexCoord.y)) * (0.15);
