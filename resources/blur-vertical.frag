...
const float blurSize = 0.0005;//1.0/512.0;
...
void main(void)
{
	...
	sum += texture2D(tex0, vec2(vTexCoord.x, vTexCoord.y + 4.0*blurSize)) * (0.05+0.03);
	...
	sum += texture2D(tex0, vec2(vTexCoord.x, vTexCoord.y + blurSize)) * (0.15);
